...
export const deleteAccount = () => async (dispatch) => {
  if (
    ...
    try {
      ...
      dispatch(
        setAlert("You account has been deleted successfully.", "success")
      );
    } catch (err) {
      dispatch({
        type: USER_ERROR,
...
// Change password
export const updatePassword = ({ oldPassword, password1 }) => async (
  dispatch
) => {
  try {
    ...
    dispatch(
      setAlert("Your password has been updated successfully", "success")
    );
  } catch (err) {
    const errors = err.response.data.errors;
